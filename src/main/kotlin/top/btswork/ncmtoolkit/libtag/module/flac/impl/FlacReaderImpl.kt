...
const val enableStrictSamples = true
...
class FlacReaderImpl : FlacReader {
  ...
  override fun Reader.sliceContent() = sliceContent(null)

  override fun Reader.sliceContent(totalSamples: Long?): FlacStream {
    ...
    while (totalSamples == null || accumulatedSamples < totalSamples) {
      ...
      if (success) {
        ...
        if (VERBOSE) println("FRAME - SYNC $position ~ $end skip=${end - position} true samples=$accumulatedSamples/$totalSamples time=${b - a}")
      } else {
  ...
  private fun Reader.confirmHeader(syncStart: Int): FrameInfo? {
    ...
    if (blockSizeCode == 0) return null
    ...
    // Read the variable-length frame/sample-number first (UTF-8 like coding)
    ...
    // Now after the variable-length field, read the optional extra bytes for blocksize/sample-rate
